...
export default function useDataTable() {
  ...
  const onEditRow = (id: number) => {
    branchStore.editId = id
    branchStore.stateModal.isShowUpdateModal = true
  }
  ...
  const columns: DataTableColumns<Tag.Item> = reactive([
    {
      ...
      width: 70,
      render: (_, index) =>
        renderIndex(branchStore.urlParams.page, branchStore.urlParams.per_page, index)
    },
    {
      title: () => 'Ngành',
      key: 'name',
