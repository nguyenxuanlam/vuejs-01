...
export const useCheckDomainStore = defineStore('checkDomainStore_proxy', {
  state: () => ({
    listData: [] as CheckDomain[],
    pagination: {
      totalItem: 0,
      pageCount: 0
    },
  ...
  actions: {
    async fetchData() {
      ...
      try {
        ...
        const res = await axios.get<CheckDomain[] | null>('/task', { params: this.urlParams })
        if (res.data === null) {
          this.listData = []
        } else {
    ...
    async retryById(id: string) {
      try {
        ...
        if (index !== -1) {
          ...
          this.listData[index].proxies = null
